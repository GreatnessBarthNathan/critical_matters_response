...
export default function DashboardPage() {
  ...
  const waiting = pastor
    ? (stats?.new ?? 0) + (stats?.awaitingPastor ?? 0)
    : (stats?.awaitingLeader ?? 0)
  ...
  return (
    <div className='dashboard-page'>
      ...
      <section className='stat-pair' aria-label='Summary'>
        ...
        <article className='stat-tile stat-tile--accent'>
          <span className='stat-tile__label'>
            {pastor ? 'Needs your reply' : 'New replies'}
          </span>
      ...
      {!pastor && (
        <Link
          className='button button--primary button--full create-action'
          to='/app/reports/new'
        >
      ...
      <section className='panel section-panel'>
        ...
        {loading ? (
          ...
          <p className='empty-note'>
            This list could not be loaded. The message above explains why.
          </p>
