...
const PATHS = {
  ...
  assisted: {
    label: 'My pastor gave me a code',
    endpoint: '/auth/assisted-reset',
    codeField: 'resetCode',
    codeLabel: 'Reset code from your pastor',
    hint: 'Ask your pastor to issue a reset code. It expires shortly after being created.',
  },
};
    ...
    >
      <div className="segmented" role="group" aria-label="Recovery method">
        {Object.entries(PATHS).map(([key, option]) => (
          <button
            ...
            onClick={() => { setPath(key); setState({ error: '', success: '', loading: false }); }}
          >
            {option.label}
          </button>
        ))}
      ...
      <form className="stack-form" onSubmit={submit}>
        ...
        <button className="button button--primary button--full" disabled={state.loading}>
          {state.loading ? <span className="button-spinner" /> : 'Set new password'}
        </button>
      </form>
    </AuthLayout>
  );
