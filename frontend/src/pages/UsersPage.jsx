...
export default function UsersPage() {
  ...
  const toggle = async (item) => {
    try {
      const data = await api(`/users/${item.id}/status`, {
        method: 'PATCH',
      ...
      {resetCode && (
        <section className='panel form-card issued-link' role='note'>
          ...
          <p className='muted-note'>
            Read this to {resetCode.name} only after you are certain who you are
            speaking to. It expires{' '}
            {new Date(resetCode.expiresAt).toLocaleTimeString(undefined, {
              timeStyle: 'short',
            })}{' '}
            and works once. Passwords are never shown here.
          </p>
